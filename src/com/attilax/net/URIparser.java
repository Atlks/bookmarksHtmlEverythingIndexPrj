 ...
 *
 * <blockquote>
 * <table summary="Describes the components of a
 ...
 *
 * <blockquote>
 * <table cellspacing=2 summary="Describes categories
 ...
 * {@code 'a'}&nbsp;through&nbsp;{@code 'z'}</td>
 * </tr>
 * <tr>
 * <th valign=top><i>digit</i></th>
 * <td>The US-ASCII decimal digit characters,
 * {@code '0'}&nbsp;through&nbsp;{@code '9'}</td>
 * </tr>
 * <tr>
 ...
 * <li>
 * <p>
 * The {@linkplain #URIparser(java.lang.String) single-argument constructor}
 * requires any illegal characters in its argument to be quoted and preserves
 * any escaped octets and <i>other</i> characters that are present.
 * </p>
 * </li>
 *
 * <li>
 * <p>
 * The
 * {@linkplain #URIparser(java.lang.String,java.lang.String,java.lang.String,int,java.lang.String,java.lang.String,java.lang.String)
...
public class URIparser implements Comparable<URIparser>, Serializable {
	 ...
	 * parsing of URIs such as {@code "file:///foo/bar"}, which seems to be the
	 * intent of RFC&nbsp;2396 although the grammar does not permit it. If the
	 ...
	 * fragment, and can be usefully <a href="#resolve-frag">resolved</a> against a
	 * base URI.
	 *
	 * <li>
	 * <p>
	 * IPv4 addresses in host components are parsed rigorously, as specified by
	 * <a href="http://www.ietf.org/rfc/rfc2732.txt">RFC&nbsp;2732</a>: Each element
	 * of a dotted-quad address must contain no more than three decimal digits. Each
	 ...
	 * permitted to start with an <i>alphanum</i> character. This seems to be the
	 * intent of <a href="http://www.ietf.org/rfc/rfc2396.txt">RFC&nbsp;2396</a>
	 * section&nbsp;3.2.2 although the grammar does not permit it. The consequence
	 * of this deviation is that the authority component of a hierarchical URI such
	 * as {@code s://123}, will parse as a server-based authority.
	 * </p>
	 * </li>
	 *
	 * <li>
	 * <p>
	 * IPv6 addresses are permitted for the host component. An IPv6 address must be
	 * enclosed in square brackets ({@code '['} and {@code ']'}) as specified by
	 * <a href="http://www.ietf.org/rfc/rfc2732.txt">RFC&nbsp;2732</a>. The IPv6
	 * address itself must parse according to
	 * <a href="http://www.ietf.org/rfc/rfc2373.txt">RFC&nbsp;2373</a>. IPv6
	 * addresses are further constrained to describe no more than sixteen bytes of
	 * address information, a constraint implicit in RFC&nbsp;2373 but not
	 ...
	 * <li>
	 * <p>
	 * Characters in the <i>other</i> category are permitted wherever RFC&nbsp;2396
	 * permits <i>escaped</i> octets, that is, in the user-information, path, query,
	 * and fragment components, as well as in the authority component if the
	 * authority is registry-based. This allows URIs to contain Unicode characters
	 * beyond those in the US-ASCII character set.
	 * </p>
	 * </li>
	 *
	 * </ul>
	 ...
	 *
	 * <p>
	 * If a scheme is given then the path, if also given, must either be empty or
	 * begin with a slash character ({@code '/'}). Otherwise a component of the new
	 * URI may be left undefined by passing {@code null} for the corresponding
	 * parameter or, in the case of the {@code port} parameter, by passing
	 * {@code -1}.
	 *
	 * <p>
	 ...
	 * <li>
	 * <p>
	 * If a scheme is given then it is appended to the result, followed by a colon
	 * character ({@code ':'}).
	 * </p>
	 * </li>
	 *
	 * <li>
	 * <p>
	 * If user information, a host, or a port are given then the string {@code "//"}
	 * is appended.
	 * </p>
	 * </li>
	 *
	 * <li>
	 * <p>
	 * If user information is given then it is appended, followed by a commercial-at
	 * character ({@code '@'}). Any character not in the <i>unreserved</i>,
	 * <i>punct</i>, <i>escaped</i>, or <i>other</i> categories is
	 ...
	 * <li>
	 * <p>
	 * If a host is given then it is appended. If the host is a literal IPv6 address
	 * but is not enclosed in square brackets ({@code '['} and {@code ']'}) then the
	 * square brackets are added.
	 * </p>
	 * </li>
	 *
	 * <li>
	 * <p>
	 * If a port number is given then a colon character ({@code ':'}) is appended,
	 * followed by the port number in decimal.
	 * </p>
	 * </li>
	 *
	 * <li>
	 ...
	 * <i>unreserved</i>, <i>punct</i>, <i>escaped</i>, or <i>other</i> categories,
	 * and not equal to the slash character ({@code '/'}) or the commercial-at
	 ...
	 * <li>
	 * <p>
	 * If a query is given then a question-mark character ({@code '?'}) is appended,
	 * followed by the query. Any character that is not a
	 ...
	 * {@link #URIparser(String)} constructor and then invoking the
	 ...
	 *                            the given components violates RFC&nbsp;2396, or if
	 *                            the authority component of the string is present
	 *                            but cannot be parsed as a server-based authority
	 */
	public URIparser(String scheme, String userInfo, String host, int port, String path, String query, String fragment)
	 ...
	 * <li>
	 * <p>
	 * If an authority is given then the string {@code "//"} is appended, followed
	 * by the authority. If the authority contains a literal IPv6 address then the
	 * address must be enclosed in square brackets ({@code '['} and {@code ']'}).
	 * Any character not in the <i>unreserved</i>, <i>punct</i>, <i>escaped</i>, or
	 * <i>other</i> categories, and not equal to the commercial-at character
	 * ({@code '@'}), is <a href="#quote">quoted</a>.
	 * </p>
	 * </li>
	 *
	 * <li>
	 ...
	 *                            but cannot be parsed as a server-based authority
	 */
	public URIparser(String scheme, String authority, String path, String query, String fragment)
			throws URISyntaxException {
	 ...
	 *
	 * <blockquote> {@code new}
	 * {@link #URIparser(String, String, String, int, String, String, String)
	 * URI}{@code (scheme, null, host, -1, path, null, fragment);} </blockquote>
	 *
	 * @param scheme   Scheme name
	 * @param host     Host name
	 * @param path     Path
	 ...
	 */
	public URIparser(String scheme, String host, String path, String fragment) throws URISyntaxException {
	 ...
	 *
	 * <p>
	 * This constructor first builds a URI in string form using the given components
	 * as follows:
	 * </p>
	 *
	 * <ol>
	 ...
	 * <li>
	 * <p>
	 * If a scheme-specific part is given then it is appended. Any character that is
	 * not a <a href="#legal-chars">legal URI character</a> is
	 * <a href="#quote">quoted</a>.
	 * </p>
	 * </li>
	 *
	 ...
	 * <p>
	 * Finally, if a fragment is given then a hash character ({@code '#'}) is
	 * appended to the string, followed by the fragment. Any character that is not a
	 ...
	 * instance as if by invoking the {@link #URIparser(String)} constructor; this
	 * may cause a {@link URISyntaxException} to be thrown.
	 * </p>
	 *
	 * @param scheme   Scheme name
	 ...
	 *                            components violates RFC&nbsp;2396
	 */
	public URIparser(String scheme, String ssp, String fragment) throws URISyntaxException {
	 ...
	 * {@link #URIparser(String)} constructor; any {@link URISyntaxException} thrown
	 ...
	 * given string is a legal URI, for example for URI constants declared within in
	 * a program, and so it would be considered a programming error for the string
	 * not to parse as such. The constructors, which throw
	 * {@link URISyntaxException} directly, should be used situations where a URI is
	 * being constructed from user input or from some other source that may be prone
	 * to errors.
	 * </p>
	 *
	 * @param str The string to be parsed into a URI
	 ...
	 */
	public static URIparser create(String str) {
		try {
	 ...
	 * <a href="http://www.ietf.org/rfc/rfc2396.txt">RFC&nbsp;2396</a> cannot always
	 * distinguish a malformed server-based authority from a legitimate
	 * registry-based authority. It must therefore treat some instances of the
	 * former as instances of the latter. The authority component in the URI string
	 * {@code "//foo:bar"}, for example, is not a legal server-based authority but
	 * it is legal as a registry-based authority.
	 *
	 * <p>
	 * In many common situations, for example when working URIs that are known to be
	 * either URNs or URLs, the hierarchical URIs being used will always be
	 * server-based. They therefore must either be parsed as such or treated as an
	 * error. In these cases a statement such as
	 *
	 * <blockquote>
	 ...
	 *
	 * <p>
	 * can be used to ensure that <i>u</i> always refers to a URI that, if it has an
	 * authority component, has a server-based authority with proper
	 * user-information, host, and port components. Invoking this method also
	 * ensures that if the authority could not be parsed in that way then an
	 * appropriate diagnostic message can be issued based upon the exception that is
	 * thrown.
	 * </p>
	 *
	 ...
	 */
	public URIparser parseServerAuthority() throws URISyntaxException {
	 ...
	 * URI except that its path is computed by normalizing this URI's path in a
	 ...
	 * <li>
	 * <p>
	 * If a {@code ".."} segment is preceded by a non-{@code ".."} segment then both
	 * of these segments are removed. This step is repeated until it is no longer
	 * applicable.
	 * </p>
	 * </li>
	 *
	 ...
	 * ({@code ':'}), then a {@code "."} segment is prepended. This prevents a
	 * relative URI with a path such as {@code "a:b/c/d"} from later being re-parsed
	 * as an opaque URI with a scheme of {@code "a"} and a scheme-specific part of
	 * {@code "b/c/d"}. <b><i>(Deviation from RFC&nbsp;2396)</i></b>
	 * </p>
	 * </li>
	 *
	 * </ol>
	 *
	 * <p>
	 * A normalized path will begin with one or more {@code ".."} segments if there
	 * were insufficient non-{@code ".."} segments preceding them to allow their
	 * removal. A normalized path will begin with a {@code "."} segment if one was
	 * inserted by step 3 above. Otherwise, a normalized path will not contain any
	 * {@code "."} or {@code ".."} segments.
	 * </p>
	 *
	 * @return A URI equivalent to this URI, but whose path is in normal form
	 */
	public URIparser normalize() {
	 ...
	 *
	 * <p>
	 * <a name="resolve-frag"></a> If the given URI's fragment component is defined,
	 * its path component is empty, and its scheme, authority, and query components
	 * are undefined, then a URI with the given fragment but with all other
	 * components equal to those of this URI is returned. This allows a URI
	 * representing a standalone fragment reference, such as {@code "#foo"}, to be
	 ...
	 * <li>
	 * <p>
	 * A new URI is constructed with this URI's scheme and the given URI's query and
	 * fragment components.
	 * </p>
	 * </li>
	 *
	 * <li>
	 * <p>
	 * If the given URI has an authority component then the new URI's authority and
	 * path are taken from the given URI.
	 * </p>
	 * </li>
	 *
	 * <li>
	 * <p>
	 * Otherwise the new URI's authority component is copied from this URI, and its
	 ...
	 * <li>
	 * <p>
	 * If the given URI's path is absolute then the new URI's path is taken from the
	 * given URI.
	 * </p>
	 * </li>
	 *
	 ...
	 * computed by resolving the path of the given URI against the path of this URI.
	 * This is done by concatenating all but the last segment of this URI's path, if
	 * any, with the given URI's path and then normalizing the result as if by
	 * invoking the {@link #normalize() normalize} method.
	 * </p>
	 * </li>
	 *
	 * </ol>
	 ...
	 */
	public URIparser resolve(URIparser uri) {
	 ...
	 *
	 * <p>
	 * This convenience method works as if invoking it were equivalent to evaluating
	 * the expression {@link #resolve(java.net.URI)
	 ...
	 * @throws IllegalArgumentException If the given string violates RFC&nbsp;2396
	 */
	public URIparser resolve(String str) {
		return resolve(URIparser.create(str));
	}
	 ...
	 * this URI is not a prefix of the path of the given URI, then the given URI is
	 * returned.
	 * </p>
	 * </li>
	 *
	 ...
	 * computed by removing this URI's path from the beginning of the given URI's
	 * path.
	 * </p>
	 * </li>
	 *
	 ...
	 * @throws NullPointerException If {@code uri} is {@code null}
	 */
	public URIparser relativize(URIparser uri) {
	 ...
	 * <p>
	 * This convenience method works as if invoking it were equivalent to evaluating
	 * the expression {@code new URL(this.toString())} after first checking that
	 ...
	 * <i>alphanum</i> category and in the string {@code "-.+"}. A scheme always
	 * starts with an <i>alpha</i> character.
	 * <p>
	 *
	 * The scheme component of a URI cannot contain escaped octets, hence this
	 * method does not perform any decoding.
	 *
	 * @return The scheme component of this URI, or {@code null} if the scheme is
	 *         undefined
	 */
	...
	/**
	 * Returns the raw scheme-specific part of this URI. The scheme-specific part is
	 ...
	 * {@link #getRawSchemeSpecificPart() getRawSchemeSpecificPart} method except
	 * that all sequences of escaped octets are <a href="#decode">decoded</a>.
	 * </p>
	 *
	 ...
	 *
	 * <p>
	 * The authority component of a URI, if defined, only contains the commercial-at
	 * character ({@code '@'}) and characters in the <i>unreserved</i>,
	 * <i>punct</i>, <i>escaped</i>, and <i>other</i> categories. If the authority
	 * is server-based then it is further constrained to have valid
	 * user-information, host, and port components.
	 * </p>
	 *
	 ...
	 * <p>
	 * The string returned by this method is equal to that returned by the
	 * {@link #getRawAuthority() getRawAuthority} method except that all sequences
	 * of escaped octets are <a href="#decode">decoded</a>.
	 * </p>
	 *
	 * @return The decoded authority component of this URI, or {@code null} if the
	 *         authority is undefined
	 */
	 ...
	 *
	 * <p>
	 * The user-information component of a URI, if defined, only contains characters
	 * in the <i>unreserved</i>, <i>punct</i>, <i>escaped</i>, and <i>other</i>
	 * categories.
	 * </p>
	 *
	 * @return The raw user-information component of this URI, or {@code null} if
	 ...
	 * <p>
	 * The string returned by this method is equal to that returned by the
	 * {@link #getRawUserInfo() getRawUserInfo} method except that all sequences of
	 * escaped octets are <a href="#decode">decoded</a>.
	 * </p>
	 *
	 * @return The decoded user-information component of this URI, or {@code null}
	 ...
	 * ({@code '-'}), though hyphens never occur as the first or last characters in
	 * a label. The rightmost label of a domain name consisting of two or more
	 * labels, begins with an <i>alpha</i> character.</li>
	 *
	 * <li>
	 * <p>
	 ...
	 * <li>
	 * <p>
	 * An IPv6 address enclosed in square brackets ({@code '['} and {@code ']'}) and
	 * consisting of hexadecimal digits, colon characters ({@code ':'}), and
	 * possibly an embedded IPv4 address. The full syntax of IPv6 addresses is
	 * specified in <a href="http://www.ietf.org/rfc/rfc2373.txt"><i>RFC&nbsp;2373:
	 * IPv6 Addressing Architecture</i></a>.
	 * </p>
	 * </li>
	 *
	 * </ul>
	 *
	 * The host component of a URI cannot contain escaped octets, hence this method
	 ...
	 * ({@code '/'}), the commercial-at character ({@code '@'}), and characters in
	 * the <i>unreserved</i>, <i>punct</i>, <i>escaped</i>, and <i>other</i>
	 * categories.
	 * </p>
	 *
	 ...
	 * <p>
	 * The string returned by this method is equal to that returned by the
	 * {@link #getRawPath() getRawPath} method except that all sequences of escaped
	 * octets are <a href="#decode">decoded</a>.
	 * </p>
	 *
	 * @return The decoded path component of this URI, or {@code null} if the path
	 *         is undefined
	 */
...
//        return decodedPath;
	 ...
	 * </p>
	 *
	 * @return The raw query component of this URI, or {@code null} if the query is
	 *         undefined
	 */
	 ...
	 * </p>
	 *
	 * @return The decoded query component of this URI, or {@code null} if the query
	 *         is undefined
	 */
	 ...
	 * <p>
	 * The string returned by this method is equal to that returned by the
	 * {@link #getRawFragment() getRawFragment} method except that all sequences of
	 * escaped octets are <a href="#decode">decoded</a>.
	 * </p>
	 *
	 * @return The decoded fragment component of this URI, or {@code null} if the
	 *         fragment is undefined
	 */
	 ...
	 *
	 * <p>
	 * For two URIs to be considered equal requires that either both are opaque or
	 * both are hierarchical. Their schemes must either both be undefined or else be
	 * equal without regard to case. Their fragments must either both be undefined
	 * or else be equal.
	 *
	 * <p>
	 * For two opaque URIs to be considered equal, their scheme-specific parts must
	 * be equal.
	 *
	 * <p>
	 * For two hierarchical URIs to be considered equal, their paths must be equal
	 * and their queries must either both be undefined or else be equal. Their
	 * authorities must either both be undefined, or both be registry-based, or both
	 * be server-based. If their authorities are defined and are registry-based,
	 * then they must be equal. If their authorities are defined and are
	 * server-based, then their hosts must be equal without regard to case, their
	 * port numbers must be equal, and their user-information components must be
	 * equal.
	 *
	 * <p>
	 * When testing the user-information, path, query, fragment, authority, or
	 * scheme-specific parts of two URIs for equality, the raw forms rather than the
	 * encoded forms of these components are compared and the hexadecimal digits of
	 * escaped octets are compared without regard to case.
	 *
	 * <p>
	...
	public boolean equals(Object ob) {
		if (ob == this)
			return true;
		if (!(ob instanceof URIparser))
			return false;
		URIparser that = (URIparser) ob;
	...
	/**
	 * Returns a hash-code value for this URI. The hash code is based upon all of
	 ...
	 * undefined but the other is defined then the first is considered to be less
	 * than the second. Unless otherwise noted, string components are ordered
	 * according to their natural, case-sensitive ordering as defined by the
	 * {@link java.lang.String#compareTo(Object) String.compareTo} method. String
	 * components that are subject to encoding are compared by comparing their raw
	 * forms rather than their encoded forms.
	 *
	 * <p>
	 ...
	 * <li>
	 * <p>
	 * Two URIs with different schemes are ordered according the ordering of their
	 * schemes, without regard to case.
	 * </p>
	 * </li>
	 *
	 * <li>
	 ...
	 * <li>
	 * <p>
	 * Two opaque URIs with identical schemes are ordered according to the ordering
	 * of their scheme-specific parts.
	 * </p>
	 * </li>
	 *
	 * <li>
	 * <p>
	 * Two opaque URIs with identical schemes and scheme-specific parts are ordered
	 * according to the ordering of their fragments.
	 * </p>
	 * </li>
	 *
	 * <li>
	 ...
	 * identical then the URIs are ordered according to the ordering of their hosts,
	 * without regard to case; if the hosts are identical then the URIs are ordered
	 * according to the ordering of their ports.
	 * </p>
	 * </li>
	 *
	 * <li>
	 ...
	 * paths are identical then they are ordered according to the ordering of their
	 * queries; if the queries are identical then they are ordered according to the
	 * order of their fragments.
	 * </p>
	 * </li>
	 *
	 * </ul>
	 ...
	 *
	 * @return A negative integer, zero, or a positive integer as this URI is less
	 *         than, equal to, or greater than the given URI
	 *
	 * @throws ClassCastException If the given object is not a URI
	 */
	public int compareTo(URIparser that) {
		int c;
	 ...
	 *
	 * <p>
	 * If this URI does not contain any characters in the <i>other</i> category then
	 * an invocation of this method will return the same value as an invocation of
	 * the {@link #toString() toString} method. Otherwise this method works as if by
	 * invoking that method and then <a href="#encode">encoding</a> the result.
	 * </p>
	 *
	 ...
	 *
	 * <p>
	 * The only serializable field of a URI instance is its {@code string} field.
	 * That field is given a value, if it does not have one already, and then the
	 * {@link java.io.ObjectOutputStream#defaultWriteObject()} method of the given
	 * object-output stream is invoked.
	 * </p>
	 *
	 ...
	 *
	 * <p>
	 * The {@link java.io.ObjectInputStream#defaultReadObject()} method is invoked
	 * to read the value of the {@code string} field. The result is then parsed in
	 * the usual way.
	 *
	...
	// RFC2396 5.2
	private static URIparser resolve(URIparser base, URIparser child) {
		...
		URIparser ru = new URIparser(); // Resolved URI
		ru.scheme = base.scheme;
	...
	//
	private static URIparser normalize(URIparser u) {
	...
	//
	private static URIparser relativize(URIparser base, URIparser child) {
		// check if child if opaque first so that NPE is thrown
		// if child is null.
		if (child.isOpaque() || base.isOpaque())
			return child;
	...
	private class Parser {
		...
		//		fragment = substring(p + 1, n);
			...
			try {
				...
				if (protoIndex < 1)
					throw new RuntimeException("no protocal::");
				...
				String[] aStrings2= this.input.split("@");
			...
			//	String hostportString =aStrings2[1];
				...
				port =getPort(this.input);//    Integer.parseInt(input.substring(hostEnd + 1, portEnd));
				System.out.println("");
			} catch (Exception e) {
				e.printStackTrace();
			}
		...
		private int getPort(String input2) {
			if(hasUserinfo(input2)) {
				...
				if (portEnd == -1)
					portEnd = input.length();
				return Integer.parseInt(input.substring(hostEnd + 1, portEnd));
			}
		}
	...
	public List<NameValuePair> getQueryParams() {
		List<NameValuePair> list = Lists.newLinkedList();
		int queryPos = this.uri.indexOf("?");
		String queryString = this.uri.substring(queryPos+1);
		String[] aStrings = queryString.split("&");
		for (String nvp : aStrings) {
			NameValuePair nvpair = getNameValuePair(nvp);
			list.add(nvpair);
		}
		return list;
	}

	private NameValuePair getNameValuePair(String nvp) {
		String[] aStrings = nvp.split("=");
		BasicNameValuePair BasicNameValuePair1 = new BasicNameValuePair(aStrings[0], aStrings[1]);
		return BasicNameValuePair1;
	}

	public String getUrlPrepart() {
		int queryPos = this.uri.indexOf("?");
		return this.uri.substring(0, queryPos);
	}
